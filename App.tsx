...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (activeSession) {
      ...
      if (savedChat) {
        ...
        setMessages([]);
        // بدء محادثة جديدة تلقائياً إذا كانت فارغة
  ...
  const deleteSession = (e: React.MouseEvent, sessionId: string) => {
    e.stopPropagation(); // منع فتح الجلسة عند الضغط على زر الحذف
  ...
  const clearAllHistory = () => {
    if (confirm('هل أنت متأكد من مسح السجل بالكامل؟ لا يمكن تراجع عن هذه الخطوة.')) {
  ...
  const handleSendMessage = async (textOverride?: string, isAuto: boolean = false) => {
    ...
    setIsTyping(true);
    try {
      // نستخدم المود المخزن في الجلسة نفسها لضمان استمرار التصحيح في وضع الاختبار
  ...
  const getSubjectsByGrade = (grade: GradeLevel): Subject[] => {
    if (grade.includes('ابتدائي')) return ['قرآن كريم', 'تربية إسلامية', 'لغة عربية', 'رياضيات', 'علوم', 'دراسات اجتماعية'];
    if (grade.includes('علمي')) return ['قرآن كريم', 'تربية إسلامية', 'لغة عربية', 'لغة إنجليزية', 'رياضيات', 'فيزياء', 'كيمياء', 'أحياء'];
    if (grade.includes('أدبي')) return ['قرآن كريم', 'تربية إسلامية', 'لغة عربية', 'لغة إنجليزية', 'تاريخ', 'جغرافيا', 'علم اجتماع', 'فلسفة ومنطق'];
    return ['قرآن كريم', 'تربية إسلامية', 'لغة عربية', 'لغة إنجليزية', 'رياضيات', 'علوم', 'تاريخ', 'جغرافيا'];
  };

  return (
    <div className="h-screen w-full flex items-center justify-center p-2 md:p-6 overflow-hidden">
      <div className="glass-card w-full max-w-6xl h-full md:h-[90vh] rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row overflow-hidden border border-white/40">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 scrollbar-hide no-select">
            <div className="flex items-center justify-between mb-2">
               ...
               {savedSessions.length > 0 && (
                 <button onClick={clearAllHistory} className="text-[9px] text-red-300 hover:text-red-100 font-bold transition-colors">مسح الكل</button>
               )}
            </div>
            {savedSessions.length === 0 ? (
              ...
              savedSessions.map(s => (
                <div key={s.id} className="group relative">
                  ...
                  <button onClick={(e) => deleteSession(e, s.id)} className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-red-500/20 hover:bg-red-500 rounded-lg opacity-0 group-hover:opacity-100 transition-all">
        ...
        {/* Main Content Area */}
        <div className="flex-1 flex flex-col overflow-hidden relative bg-white/10">
          {view === 'chat' && activeSession ? (
            <div className="flex flex-col h-full">
              <header className="px-6 py-4 flex items-center justify-between border-b border-black/5 shrink-0 bg-white/60 backdrop-blur-md z-10">
                <div className="flex items-center gap-4">
                  <button onClick={() => setView('home')} className="p-2.5 bg-indigo-600 text-white rounded-xl shadow-lg active:scale-90 transition-transform"><svg className="w-5 h-5 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg></button>
                  <div>
                    <h2 className="text-sm font-black text-indigo-950">{activeSession.subject}</h2>
                    <p className="text-[10px] text-slate-500 font-bold">{activeSession.grade} • {activeSession.mode === 'test' ? 'اختبار وتصحيح' : 'شرح تعليمي'}</p>
                  </div>
                </div>
                <button onClick={(e) => deleteSession(e, activeSession.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-xl transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7" /></svg></button>
              </header>
              <div className="flex-1 relative overflow-hidden">
                <ChatWindow messages={messages} isTyping={isTyping} />
              </div>
              <footer className="p-4 bg-white/80 border-t border-black/5 backdrop-blur-md">
                <div className="max-w-4xl mx-auto">
                  ...
                  <div className="flex items-end gap-3 bg-slate-100/80 p-2 rounded-2xl shadow-inner border border-black/5 focus-within:bg-white transition-all">
                    ...
                    <textarea value={inputText} onChange={(e)=>setInputText(e.target.value)} placeholder="أجب على السؤال أو اسأل الأستاذ..." className="flex-1 bg-transparent border-none focus:ring-0 text-sm font-bold resize-none p-2 min-h-[44px]" rows={1} onKeyDown={(e)=>e.key==='Enter' && !e.shiftKey && (e.preventDefault(), handleSendMessage())} />
                    <button onClick={()=>handleSendMessage()} disabled={isTyping || (!inputText.trim() && !selectedImage)} className="bg-indigo-600 text-white p-4 rounded-xl shadow-lg hover:bg-indigo-700 disabled:bg-slate-300 active:scale-95 transition-all">{isTyping ? <div className="w-5 h-5 border-2 border-white/30 border-t-white animate-spin rounded-full"/> : <svg className="w-5 h-5 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>}</button>
                  </div>
                </div>
              </footer>
            </div>
            ...
            <div className="flex-1 flex flex-col p-6 md:p-12 overflow-y-auto scrollbar-hide">
              {view === 'home' && (
                <div className="flex flex-col items-center justify-start min-h-full space-y-10 animate-in fade-in duration-700 pt-4">
                  <div className="w-24 h-24 bg-indigo-600 rounded-[2.2rem] flex items-center justify-center text-white text-5xl shadow-[0_20px_50px_rgba(79,70,229,0.3)] animate-bounce">👨‍🏫</div>
                  <div className="space-y-2 text-center">
                    <h2 className="text-3xl md:text-5xl font-black text-indigo-950 tracking-tighter">الأستاذ سورا</h2>
                    <p className="text-slate-600 font-bold text-xs md:text-lg italic">خبير المناهج اليمنية الذكي</p>
                  </div>
                  ...
                  {/* سجل المحادثات (للموبايل والديسكتوب في الصفحة الرئيسية) */}
                  <div className="w-full max-w-2xl px-2 space-y-4">
                    <div className="flex items-center justify-between border-b border-black/5 pb-2 px-2">
                       ...
                       {savedSessions.length > 0 && (
                         <button onClick={clearAllHistory} className="text-[10px] text-red-500 font-black hover:underline">مسح الكل</button>
                       )}
                    </div>
                    {savedSessions.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {savedSessions.slice(0, 6).map(s => (
                          <div key={s.id} className="relative group">
                            <button onClick={() => setActiveSession(s)} className="w-full p-4 glass-card rounded-2xl flex items-center gap-3 text-right hover:border-indigo-500 transition-all active:scale-95 group/btn overflow-hidden">
                              <span className="text-2xl">{SUBJECT_DETAILS[s.subject]?.icon || '📚'}</span>
                              <div className="flex-1 overflow-hidden">
                                <p className="text-xs font-black text-slate-800 truncate">{s.subject}</p>
                                <p className="text-[9px] text-slate-500 font-bold">{s.grade}</p>
                              </div>
                            </button>
                            <button onClick={(e) => deleteSession(e, s.id)} className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-red-100 text-red-600 rounded-lg opacity-0 group-hover:opacity-100 md:group-hover:opacity-100 transition-all z-10">
                              <svg className="w-3.2 h-3.2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="pt-4 pb-10"><a href={DEV_WHATSAPP} target="_blank" rel="noopener noreferrer" className="text-emerald-600 font-black text-[11px] bg-emerald-50 px-4 py-2 rounded-full border border-emerald-100 flex items-center gap-2 hover:bg-emerald-100 transition-all active:scale-95"><span>💬</span> تواصل مع المطور واتساب</a></div>
                </div>
              )}

              {view === 'select-grade' && (
                <div className="animate-in slide-in-from-bottom-6 duration-500">
                  <button onClick={()=>setView('home')} className="mb-8 flex items-center gap-2 text-indigo-600 font-black text-sm hover:translate-x-1 transition-transform"> <svg className="w-4 h-4 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg> العودة للرئيسية</button>
                  <h3 className="text-3xl font-black text-indigo-950 mb-8 text-center underline decoration-indigo-200 underline-offset-8">في أي صف دراسي أنت؟</h3>
              ...
              {view === 'select-subject' && (
                <div className="animate-in slide-in-from-bottom-6 duration-500">
                  ...
                  <h3 className="text-3xl font-black text-indigo-950 mb-2 text-center">اختر المادة</h3>
