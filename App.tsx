...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (textOverride?: string, isAuto: boolean = false) => {
    ...
    try {
      ...
      console.error(e);
      const errorMsg: ChatMessage = { id: `err_${Date.now()}`, role: 'model', text: "❌ عذراً، فشل الاتصال. تأكد من الإنترنت أو مفتاح الـ API الخاص بك.", timestamp: new Date() };
        ...
        <div className="flex-1 flex flex-col min-h-0 relative bg-white/10">
          {view === 'chat' && activeSession ? (
            ...
            <div className="flex-1 flex flex-col overflow-y-auto scrollbar-hide px-6 py-8 md:p-12">
              {view === 'home' && (
                <div className="flex flex-col items-center justify-start min-h-full space-y-10 animate-in fade-in duration-700">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-10">
                        {savedSessions.slice(0, 6).map(s => (
                          <div key={s.id} className="relative group">
                            ...
                            <button onClick={(e) => deleteSession(e, s.id)} className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-red-100 text-red-600 rounded-lg opacity-0 group-hover:opacity-100 transition-all z-10">
              ...
              {view === 'select-grade' && (
                <div className="animate-in slide-in-from-bottom-6 duration-500 pb-10">
